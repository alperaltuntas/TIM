...
| 4 | **turbo-stack-iturbo** | — | **uncommitted** | nvhpc template CXXFLAGS fix + `nvcc-cxx-wrap.sh` `-Xcompiler` re-injection (OFFLOAD path) |
| 5 | **cime** (sandbox) | detached | **uncommitted** | `CIME/Tools/Makefile`: `MOM6_INFRA_API==TIM` → `-lamrex -lstdc++` after `-lfms` |
| 6 | **components/mom wrapper** (sandbox, detached-HEAD external) | detached | **uncommitted** | `config_component.xml` (TIM valid value), `buildlib` (AMReX include), `testdefs/.../mom/tim` testmod |
| 7 | **FMS interface** (sandbox `libraries/FMS`) | `add_amrex_submodule` | `1164e92` committed; Makefile.cesm + src pointer **uncommitted** | buildlib: `tim/` Filepath additions + E4 Filepath excludes; Makefile.cesm PIO/netCDF INCLDIR |
| 8 | **stochastic_physics** (MOM external) | new `parallelio-prototype` | `681fd9e` | `update_ca.F90` CA-restart fms2_io stubs (CA is atmosphere-only; ocean SKEB untouched — validated with `DO_SKEB=True` active) |

The uncommitted trees (3–6) are a known liability, itemized in §3.3 as explicit
...
- **Pre-existing bugs discovered en route** (not ours, still open): TIM restart
  ...
  fp-model gap (fixed in turbo-stack and iturbo after the Accumulator property tests
...
- **Repo hygiene:** turbo-stack/iturbo/cime/mom-wrapper changes uncommitted;
...
**Track B — MOM6 (each depends on the matching A-item):**
...
- **C1. FMS-source elimination** (re-apply E4/E5 + stochastic_physics stub). *Gate:
  ...
  netCDF object).*
- **C2. CESM wiring PRs to their own repos:** FMS-interface buildlib/Makefile.cesm,
  mom wrapper xml/buildlib/testmod (needs a proper fork/branch — currently
  detached-HEAD), cime Makefile link glue, rpointer/st_archive for `TIM.diag.res.nc`.
